    ...
    def create(self, inputs, kernel_size=None, seed=None, reuse_variables=None):
        ...
        with tf.compat.v1.variable_scope(self.name, reuse=reuse_variables):
            ...
            for index, kernel in enumerate(self.decoder_kernels):
                ...
                layer_to_concat = layers[len(layers) - index - 2]
                output_resized = tf.image.resize(output, size=tf.shape(layer_to_concat)[1:3])
                output = tf.compat.v1.concat([layers[len(layers) - index - 2], output_resized], axis=3)
